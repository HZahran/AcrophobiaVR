...
STDMETHODIMP CDevice::Open()
{
	...
	m_nDeviceId = faGetId(nDevCnt - 1); // see Alexey.
...
HRESULT CDevice::GetDeviceInfos(BSTR *pszInfo)
{
	...
	{
		szDevDescr = "-- FirstAmp -------- \r\n";
		_itoa_s(m_DeviceInfo.SerialNumber, szStr, 10);
		szDevDescr += "   SerialNo: " + CString(szStr) + "\r\n";
		sprintf_s(szStr, "   %d Channels ", m_tblChanInfo.size() - 1);
...
/*		// without trigger.
		m_pFilterAntiAliasing = new CFilterButterworth(nAnalogChannels,
			CFilterButterworth::eHighCutoff, 24, GetMaxSampleRate(), &tblFrequencies[0]);
*/
			}
	m_bFirstData = TRUE;
